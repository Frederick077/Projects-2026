  ...
  return (
    <section id="combined" className="py-20 bg-white text-[#2B2B2B] scroll-mt-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-16">
          {combinedPackages.map((pkg, index) => (
            <motion.div
              ...
              className="bg-[#F5F1E8]/40 border border-[#1F4D3A]/5 hover:border-[#C9A227]/30 hover:bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col sm:flex-row group"
            >
              ...
              <div className="sm:w-2/5 relative h-56 sm:h-auto min-h-60 shrink-0 overflow-hidden">
                ...
                {/* Best Seller Banner */}
                {pkg.isBestSeller && (
                  <div className="absolute top-4 left-4 bg-[#C96B2C] text-white text-[9px] font-bold uppercase tracking-wider px-3 py-1.5 rounded shadow-sm flex items-center gap-1">
              ...
              <div className="p-6 sm:p-8 flex flex-col justify-between flex-grow">
                <div>
                  ...
                  {/* Duration Capsule */}
                  <div className="flex items-center gap-1.5 text-xs font-mono text-[#C96B2C] font-semibold mb-3">
                    <Calendar className="w-4 h-4" />
                    <span>{pkg.duration}</span>
                  </div>

                  {/* Title */}
                  <h3 className="font-serif text-xl font-bold text-[#1F4D3A] mb-3 leading-tight group-hover:text-[#C96B2C] transition-colors duration-200">
                    {pkg.name}
                  </h3>
                ...
                <div className="flex items-center justify-between mt-auto pt-4 border-t border-[#1F4D3A]/5">
                  <div>
                    <span className="text-[10px] text-gray-400 block font-sans">Starting from</span>
                    <span className="font-serif text-xl sm:text-2xl font-bold text-[#1F4D3A]">
        ...
        <div className="bg-[#1F4D3A] text-white rounded-2xl p-6 sm:p-8 relative overflow-hidden shadow-lg border border-[#C9A227]/40">
          <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="flex items-center gap-4">
              <div className="bg-[#C9A227] p-3 text-[#1F4D3A] rounded-2xl shadow-md shrink-0">
                <Camera className="w-6 h-6 stroke-[2]" />
              </div>
              <div>
                <h4 className="font-serif text-lg sm:text-xl font-bold text-[#F5F1E8]">
            ...
            <span className="shrink-0 font-mono text-[10px] sm:text-xs uppercase bg-[#C9A227]/15 text-[#C9A227] rounded-full border border-[#C9A227]/40 px-5 py-2.5 tracking-wider font-bold">
              Included Free In All Premium Bookings
            </span>
          </div>

          {/* Underlay glow */}
          <div className="absolute right-0 top-0 bottom-0 w-1/3 bg-radial from-[#C96B2C]/20 to-transparent pointer-events-none" />
        </div>
