...
export default function SafariSection({ onOpenBooking }: SafariSectionProps) {
  return (
    <section id="safaris" className="py-20 bg-gradient-to-b from-[#133024] to-[#1F4D3A] text-white scroll-mt-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <p className="text-base sm:text-lg text-[#F5F1E8]/75 leading-relaxed">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12 max-w-5xl mx-auto">
          {[
            ...
            <div key={index} className="bg-white/5 border border-white/10 rounded-xl p-5 text-center">
              <span className="block font-sans font-bold text-[#C9A227] text-sm tracking-wider uppercase mb-2">
                {hl.tag}
              </span>
              <p className="text-xs sm:text-sm text-[#F5F1E8]/80 leading-relaxed">
                {hl.value}
              </p>
            </div>
          ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-stretch">
          {wildlifePackages.map((pkg, index) => (
            <motion.div
              ...
              className="flex flex-col bg-white text-[#2B2B2B] rounded-2xl overflow-hidden border border-[#C9A227]/20 hover:border-[#C9A227]/60 shadow-lg hover:shadow-2xl transition-all duration-300 group"
            >
              ...
              <div className="relative h-60 overflow-hidden shrink-0">
                ...
                {pkg.isPopular && (
                  <div className="absolute top-4 right-4 bg-[#C96B2C] text-white text-[10px] font-bold uppercase tracking-widest py-1 px-3 rounded-full flex items-center gap-1">
              ...
              <div className="p-6 flex flex-col flex-grow">
                ...
                {/* Parks highlights */}
                <div className="mb-4">
                  <span className="text-[10px] uppercase font-mono tracking-wider text-[#C96B2C] block mb-2">
                    Destinations:
                  </span>
                  <div className="flex flex-wrap gap-1.5">
                    {pkg.parks.map((park, i) => (
                      <span
                        key={i}
                        className="bg-[#1F4D3A]/5 text-[#1F4D3A] text-xs font-semibold px-2.5 py-1 rounded-md flex items-center gap-1 border border-[#1F4D3A]/5"
                      >
                        <MapPin className="w-3 h-3 text-[#C96B2C]" />
                        {park}
                      </span>
                    ))}
                  </div>
                  ...
                  <ul className="space-y-1 lg:space-y-1.5">
                    {pkg.included.slice(0, 4).map((inc, i) => (
                      <li key={i} className="text-xs text-[#2B2B2B]/85 leading-relaxed flex items-start gap-2">
                        <Check className="w-3.5 h-3.5 text-[#1F4D3A] shrink-0 mt-0.5" />
                        <span>{inc}</span>
                      </li>
                    ))}
                  </ul>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[#1F4D3A] text-xs font-mono font-bold border border-[#1F4D3A]/20 rounded-lg px-3 py-1 bg-[#1F4D3A]/5">
                    Private Guiding
                  </span>
                </div>
